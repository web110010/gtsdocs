...
Local media should be backed up. You can use the [GoToSocial CLI](../cli#gotosocial-admin-media-list-local) to list all media files that belong to your instance and its users.
...
For information on how to use the commands to import/export, see [here](../cli#gotosocial-admin-export). Though the `export` command won't backup media, you can use the [`media list-local`](cli.md#gotosocial-admin-media-list-local) command to figure out which media files you should keep.
...
Use the [GoToSocial CLI](../cli#gotosocial-admin-media-list-local) to get a list of media files you need to safeguard.
...
The file mentioned in `patterns_from` can be created by transforming the output from the [GoToSocial CLI](../cli#gotosocial-admin-media-list-local). In order to generate the right patterns you can use the [`media-to-borg-patterns.py`](https://github.com/superseriousbusiness/gotosocial/tree/main/example/borgmatic/media-to-borg-patterns.py) script. How Borg patterns work is explained in [their documentation](https://man.archlinux.org/man/borg-patterns.1).
...
For this to work reliably, you should ensure that the [storage-local-base-path](../../configuration/storage) in your GoToSocial configuration uses an absolute path. Otherwise you'll have to tweak the paths yourself.
{{</hint>}}
